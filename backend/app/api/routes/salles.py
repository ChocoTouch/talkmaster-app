...
async def create_salle(salle: SalleCreate, current_user=Depends(get_current_user)):
    ...
    if current_user.id_role != 4:  # Assurez-vous que `current_user` a l'attribut `id_role`
        raise HTTPException(status_code=403, detail="Accès interdit. Vous devez être administrateur pour créer des salles.")
